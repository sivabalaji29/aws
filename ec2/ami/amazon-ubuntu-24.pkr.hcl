// Define Packer settings for AWS EC2 plugin
packer {
  required_plugins {
    amazon = {
      version = "1.7.0"
...
// Run source Amazon EC2 instance to create new AMI.
source "amazon-ebs" "ec2_ubuntu_inst" {
  ami_name                    = "sloopstash-ubuntu-22.04-v1.1.1-ami"
  region                      = var.region
...
build {
  name    = "ec2_ubuntu_ami"
  sources = ["source.amazon-ebs.ec2_ubuntu_inst"]
